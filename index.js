    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (orderStates[sender]?.step === 'WAITING_FOR_ADDRESS') {
            ...
            const javaGoatOrder = {
                ...
                userEmail: "supportsasaxcheatz@sasaxcheatz.store",
        ...
        if (text.startsWith("order ")) {
            ...
            if (!matchedItem) {
                await sock.sendMessage(sender, { text: `❌ Sorry, we couldn't find *${productRequested}*in our panel menu\n\nType*menu* to see all available items.` });
                return;
            }
            ...
            await sock.sendMessage(sender, { text: "🛒 *How to order:* \nPlease type 'order' followed by the Panel name. \nExample: *order Drip client*" });
        }
        ...
        // --- DYNAMIC MENU FEATURE ---
            ...
            let menuMessage = "👾 *SASAXCHEATZ PANEL MENU 👾* 🍕\n\n";
            currentMenu.forEach(item => {
                menuMessage += `🔸 *${item.name}* - ₹${item.price}\n`;
            });
            menuMessage += "\n_To order, reply with 'order [Panel Name]'_";
        ...
        // --- GREETINGS ---
            ...
            await sock.sendMessage(sender, { text: "👋 *Welcome to Sasaxcheatz!* \n\nI am your AI Assistant. Type *menu* to see our panels, or type *order [panel]* to buy instantly!" });
        }
        else if (text.includes("contact") || text.includes("call")) {
            await sock.sendMessage(sender, { text: "📞 *Contact Sasaxcheatz:* \n\n- *Email:* supportsasaxcheatz@sasaxcheatz.store" });
        }
        else {
            await sock.sendMessage(sender, { text: "🤔 I didn't quite catch that.\n\nType *menu* to see our Panels list, or *order [panel]* to place an order!" });
        }
    });
}
